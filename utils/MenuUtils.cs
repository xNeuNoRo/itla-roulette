    ...
    public class Menu
    {
        ...
        public static void AnimatedLogo()
        {
            string[] logo = new string[]
            {
                ...
                "/* ▐     | || '_ \\| __/ _ \\ '__/ _` |/ __| __| \\ \\ / / _ \\                         ▌ */",
                "/* ▐     | || | | | ||  __/ | | (_| | (__| |_| |\\ V /  __/                         ▌ */",
                "/* ▐    |___|_| |_|\\__\\___|_|  \\__,_|\\___|\\__|_| \\_/ \\___|     _           _       ▌ */",
            ...
            string[] cj = GetCJArt();
            int totalLines = Math.Max(logo.Length, cj.Length);

            for (int step = 1; step <= logo.Length; step++)
            {
                Console.Clear();

                for (int i = 0; i < totalLines; i++)
                {
                    string cjLine = i < cj.Length ? cj[i] : new string(' ', cj[0].Length);
                    string logoLine = (i < step) ? logo[i] : new string(' ', logo[0].Length);

                    Console.WriteLine($"{cjLine}        {logoLine}");
                }
